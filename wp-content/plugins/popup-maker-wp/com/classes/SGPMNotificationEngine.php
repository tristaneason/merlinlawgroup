...
class SGPMNotificationEngine
{
	...
	private $activeNotificationsCount = false;
	...
	{
		add_action('sgpm_fetch_new_notifications', array($this, 'fetchNewNotifications'));
		add_filter('cron_schedules', array($this, 'checkeForNewNotificationsCron')); // add new cron

		if (!wp_next_scheduled('checkeForNewNotificationsCron')) {
			wp_schedule_event(time(), 'sgpm_check_for_new_notifications_every_6_hours', 'sgpm_fetch_new_notifications');
		}
	}
	...
	private function getNotificationBody()
	{
		...
		if ($showReviewNotice) {
			...
			$this->activeNotificationsCount += 1;
		}
